...
sl = math.sin( 15 * math.pi / 180 )  # Schraegstellung (s.a. render.py)

# Effektive Abstaende fuer Vokale
di  = 0.6  # i, ü
de  = 1.2  # e, a, ö, ä
du  = 2.8  # u, au, eu, o, oi, ei, 
dkv = 0.6  # Konsonantenverbindung (ohne Vokal)
...
# Glyphen-Elemente
# -----------------------------------------------------------------------------------
# Verwendung von Kontrollpunkten in Bezier Splines:
# (P0)----[P1]----[P2]----(P3/Q0)----[Q1]----[Q2]----(Q3/R0)----[R1]----[R2]----(R3)
# ()=Start- und Endpunkte
# []=Kontrollpunkte
#
# Glyphen werden von Kontrollpunkt 2 bis Kontrollpunkt 1 definiert und koennen durch nachfolgende 
# Hilfsroutinen zusammengesetzt werden:
# 
# Wenn dl definiert ist, dann ist Kontrollpunkt [P1] festgelegt, ansonsten nur (P0).
# Wenn dr definiert ist, dann werden die Punkte [R2] und (R3) definiert werden.
...
def scale(g, sx, sy, s=0):
    # Kontrollpunkte in g um sx und sy skalieren
    # und Scherung um s
    return [(x * sx + (1 - y) * s, y * sy) for (x, y) in g]


def shift(g, dx, dy=0):
    # Kontrollpunkte um dx und dy verschieben
    return [(x + dx, y + dy) for (x, y) in g]


def shiftToPos(gl, dx, dy, s=0):
    return [(dx + x + y * s, dy + y) for (x, y) in gl]


def rotate_ccw(gl, r):
    return [(x*math.cos(r)-y*math.sin(r), y*math.cos(r)+x*math.sin(r)) for (x, y) in gl]
...
def SplitStiefoWord(st):
    ...
    while len(st) > 1 and st[0] in wordOffsets:
        ...
        st = st[1:]
    for z in (st.split(' ')):
        # Test auf zwei aufeinanderfolgende Vokale, 'c' in diesem Fall
        # einfuegen
...
def stiefoWortZuKurve(w):
    scaleXX = 1.5     # Skalierung in X fuer Glyphen und Zwischenraeume, \literal
    scaleY  = 0.5     # Skalierung in Y fuer Glyphen, \literal
    wrdCtrl = []      # Bezier-Kontrollpunkte
    x       = 0       # aktuelle Stiftposition in X
    y       = 0       # aktuelle Stiftposition in Y
    ...
    for i in range(0, len(ll) - 2, 2):
        dl = ll[i]      # Vokal vor Konsonant
        k  = ll[i + 1]  # Aktueller Glyph (Konsonant)
        ...
        glFunc    = glyphs[k]
        w, glCtrl = glFunc(dl, dr)            # w = X-Abst. hinter Glyph, glCtrl = Bezier-Stuetzpunkte d. Glyphen
        glCtrl    = scale(glCtrl, scaleXX, 1) # Bezier-Stuetzpunkte in X-Richtung skalieren
        w        *= scaleXX                   # skalieren

        if dl:
            dx, dy, ea = dl      # dx = dX, dy = dY, ea = eff. Abstand in X
            x += ea              # Delta X-Position des Zeichenstiftes wird von eff. Abst. des linken Vokals bestimmt
            y += dy * scaleY     # Delta Y-Position vom Y-Delta des linken Vokals * 0.5 \literal
            xpos.append((x, y))  # Zeichenstiftposition = Start des aktuellen Glyphen, Position ablegen

        glCtrl = shiftToPos(glCtrl, x, y + wrdOffsY * scaleY, sl) # Bezier-StPk an die Stiftpos. verschieben
        x += w                   # X-Position des Zeichenstiftes nochmals verschieben
        xpos.append((x, y))

        for t in glCtrl: 
            wrdCtrl.append(t)    # Kontrollpunkte des Glyphen an wrdCtrl anhaengen


    return [(x, wrdCtrl, xpos)]  # x = Aktuelle ZchStPos in X, wrdCtrl = Kontrollpunkte des Wortes und 
                                 # xpos = Startpositionen der Konsonanten
